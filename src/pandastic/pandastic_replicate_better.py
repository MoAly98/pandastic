...
def run():
    ...
    # ============= Check existing Replicas and Rules ============= #
...
# ):
#     '''
#     Method to find the relevant jobs and associated datasets that need to be replicated.
...
#         RSE to replicate to
#     rsefrom: str
#         RSE to replicate from
#     did_regex: str
#         Regex to match the dataset name. This is only
#         used if --usetask is used.
...
#                     if any(re.match(rse, er) is not None for rse in rsefrom):
#                         rule_in_rsefrom = True
#                         break

#             rule_exits = False # hack to test

#             if (not rule_exits) and rule_in_rsefrom:
#                 DSes.add(cont)
#             else:   continue
...
#     # If specified, make sure the dataset has a rule/replica/both on RSEs with fiven regex
#     rsefrom = args.rsefrom
#     did = args.did

#     # Find all PanDA jobs that are done for the user and period specified
#     _, url, data = queryPandaMonUtils.query_tasks( username=user, days=days, status='done')

#     # Tell the user the search URL if they want to look
#     print(f"INFO:: PanDAs query URL: {url}")
#     # What am I replicating
#     to_repl = find_datasets(data, regex, args.type, args.rseto, rsefrom, did)
